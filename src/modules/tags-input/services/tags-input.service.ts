...
import { FilterFn, LookupFn, LookupFnResult } from "../../search/helpers/lookup-fn";

interface ICachedArray<T> {
    [query: string]:T[];
}

export class TagsInputService<T, U> {
    ...
    public getData(callback: (err?: Error) => void = () => {
    }): void {
    ...
    // Updates the new set of results.
